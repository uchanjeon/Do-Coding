...
//document.querySelctor로 각각의 요소를 지정
 ...
 restart();
  ...
  function searchMovies(keyword) {
      const searchUrl = `https://api.themoviedb.org/3/search/movie?language=en-US&page=1&include_adult=false&query=${keyword}&api_key=${apiKey}`;

      fetch(searchUrl)
          .then(response => response.json())
          .then(data => {
              movieCardsContainer.innerHTML = ''; // 기존 카드 제거

              if (data.results && data.results.length > 0) {
                  data.results.forEach(movie => {
                      const card = createMovieCard(movie);
                      movieCardsContainer.appendChild(card); // 검색 결과 카드 추가
                  });
              } else {
  ...
  movieCardsContainer.addEventListener('click', function (event) {
      ...
      if (clickedCard) {
         ...
         // console.log(" 영화의 ID:", movieId); 테스트용 로그
          showMovieId(movieId);
      }
  });
});
